...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run a resumable BaoStock historical data backfill.")
    parser.add_argument("symbols", nargs="*", help="Optional symbols or names. Empty means all A-share/ETF/index candidates.")
    parser.add_argument("--days", type=int, default=260, help="Lookback calendar days for each symbol batch.")
    parser.add_argument("--batch-size", type=int, default=30, help="Symbols per BaoStock batch.")
    ...
    parser.add_argument("--no-universe-refresh", action="store_true", help="Skip query_all_stock before history backfill.")
    ...
    with get_db() as conn:
        job = create_baostock_backfill_job(
            conn,
            list(args.symbols),

...
def run_backtest(payload: BacktestInput) -> dict[str, Any]:
    ...
    return {
        "mode": "mock-research-backtest",
        "run_id": f"mock-bt-{payload.strategy}-{datetime.now().strftime('%Y%m%d%H%M%S')}",
...
def research_notes(payload: BacktestInput, symbols: list[str]) -> list[str]:
    return [
        "当前为 mock 研究回测，只验证产品流程和指标展示，不代表真实历史收益。",
        "正式回测必须使用复权行情、交易日历、停复牌、涨跌停、分红拆股、交易成本和滑点模型。",
        "样本只包含当前 mock 股票池，不能据此判断策略有效性；真实版本需要足够多标的和更长历史区间。",
